...
export const findAllAvailableExams = async (studentId: number) => {
  const query = `
    SELECT e.id, e.title, e.description, e.starts_at AS "startsAt", e.ends_at AS "endsAt", c.name AS "courseName"
    FROM exams e
    JOIN courses c ON e.course_id = c.id
    WHERE NOW() BETWEEN e.starts_at AND e.ends_at
      AND e.id NOT IN (SELECT exam_id FROM attempts WHERE student_id = $1);
  `;
  const result = await pool.query(query, [studentId]);
  return result.rows;
};

export const findAvailableExamById = async (examId: number) => {
  const examQuery = `
  ...
  const questionsQuery = 'SELECT id, statement, points FROM questions WHERE exam_id = $1;';
  const questionsResult = await pool.query(questionsQuery, [examId]);
  
  const questions = [];
  for (const question of questionsResult.rows) {
    const choicesQuery = 'SELECT id, label FROM choices WHERE question_id = $1;';
...
export const submitAttempt = async (examId: number, studentId: number, answersMap: Record<number, number>) => {
  const client = await pool.connect();
  
  try {
    await client.query('BEGIN');

    const questionsQuery = 'SELECT id, points FROM questions WHERE exam_id = $1;';
    ...
    for (const question of questions) {
      const chosenChoiceId = answersMap && answersMap[question.id] ? Number(answersMap[question.id]) : null;
      let isCorrect = false;
      let correctChoiceId = null;

      const choicesQuery = 'SELECT id, is_correct AS "isCorrect" FROM choices WHERE question_id = $1;';
      const choicesResult = await client.query(choicesQuery, [question.id]);
      ...
      if (chosenChoiceId && chosenChoiceId === correctChoiceId) {
        isCorrect = true;
    ...
    const insertAttemptQuery = `
      ...
      VALUES ($1, $2, $3) 
      RETURNING id, score, submitted_at AS "submittedAt";
    `;
    const attemptResult = await client.query(insertAttemptQuery, [examId, studentId, totalScore]);
    ...
    for (const item of detailedCorrection) {
      const insertAnswerQuery = `
        ...
        VALUES ($1, $2, $3);
      `;
      await client.query(insertAnswerQuery, [attemptId, item.questionId, item.chosenChoiceId]);
    }
...
export const findAllStudentResults = async (studentId: number) => {
  const query = `
    SELECT a.id AS "attemptId", a.score, a.submitted_at AS "submittedAt",
           e.id AS "examId", e.title AS "examTitle", c.name AS "courseName"
    FROM attempts a
